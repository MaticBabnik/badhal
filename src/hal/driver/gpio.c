...
void gpio_init_output(
    ...
    unit->MODER = (unit->MODER & (u32) ~(0x3ul << (index * 2ul)))
                  | (GM_Output << (index * 2));
    unit->PUPDR =
        (unit->PUPDR & (u32) ~(0x3ul << (index * 2))) | (pull << (index * 2));
    unit->OSPEEDR = (unit->OSPEEDR & (u32) ~(0x3ul << (index * 2)))
                    | (speed << (index * 2));
...
void gpio_init_alt(
    ...
    unit->MODER =
        (unit->MODER & (u32) ~(0x3ul << (index * 2))) | (GM_Alt << (index * 2));
    unit->PUPDR =
        (unit->PUPDR & (u32) ~(0x3ul << (index * 2))) | (pull << (index * 2));
    unit->OTYPER = (unit->OTYPER & (u32) ~(1 << index)) | (otype << index);
    unit->OSPEEDR = (unit->OSPEEDR & (u32) ~(0x3ul << (index * 2)))
    ...
    unit->AFR[afrIdx] =
        (unit->AFR[afrIdx] & (u32) ~(0xf << inRegOffset)) | (af << inRegOffset);
}
